...
function App(){
  ...
  const [example, setExample] = useState(CONTENT_DATA.components)
    ...
    <div className="min-h-screen bg-purple-950 p-6 flex flex-col gap-7">
      ...
      <div className="flex gap-2 items-center text-purple-300 mx-auto w-4xl">
        {/* {CARD_DATA.map((element, index)=>(
          <Button key={index} name={element.title} onClick={()=> onClick()}/>
        ))} */}
        <Button name={CONTENT_DATA.components.title} onClick={()=> setExample(CONTENT_DATA.components)}/>
        <Button name={CONTENT_DATA.jsx.title} onClick={()=> setExample(CONTENT_DATA.jsx)}/>
        <Button name={CONTENT_DATA.props.title} onClick={()=> setExample(CONTENT_DATA.props)}/>
